...
export class AuthService {
  ...
  async generateAndSaveTokens(user: any): Promise<TokenAuthDto> {
    const { token: accessToken } =
      await this.tokenService.signAccessToken(user);
    const {
      token: refreshToken,
      iat,
      exp,
    } = await this.tokenService.signRefreshToken(user);

    await this.userTokenService.update(user.id, { iat, exp });
    this.logger.debug(
      `Refresh token data successfully updated for user: ${user.id}`,
    );
  ...
  async deleteRefreshTokenData(user: any): Promise<void> {
    ...
    this.logger.debug(
      `Refresh token data successfully deleted for user: ${user.id}`,
    );
  }
}
